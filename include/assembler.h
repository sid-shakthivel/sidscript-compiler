...
private:
    ...
    const std::unordered_map<std::string, std::array<std::string, 4>> register_table = {
	...
	void emit_push(const TACInstruction &instruction);
	void emit_pop(const TACInstruction &instruction);
